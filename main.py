...
import re
# 从agents模块导入故事板生成函数
from src.agents.planner import generate_storyboard
# 从agents模块导入代码生成和修复函数
from src.agents.coder import generate_code, fix_code, refine_code
from src.agents.asset_manager import AssetManager
from src.agents.critic import VisualCritic
from src.config import USE_VISUAL_FEEDBACK
...
def main():
    ...
    with open(storyboard_path, "w") as f:
        json.dump(storyboard, f, indent=2, ensure_ascii=False)
    print(f"✅ Enhanced storyboard saved")
    ...
    for section in storyboard.get("sections", []):
        ...
        if code:
            ...
            if args.render:
                ...
                env = os.environ.copy()
                # 设置PYTHONPATH为当前工作目录，确保可以导入src.manim_base模块
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        print(f"Error details:\n{error_output[-500:]}...") # Print last 500 chars of error
